...
Tarot-Helper 是一个本地优先的塔罗解读启示与辅助工具。它帮助用户选择牌阵、抽牌、查看牌义、统计牌面结构，并通过 DeepSeek 生成“可供继续联想的解读启示”，而不是替用户做绝对判断。

核心定位：

- **辅助**：把牌阵、牌位、正逆位、元素统计和牌义整理到同一工作台里，减少手动查找和记录成本。
- **启示**：AI 输出用于激发观察角度，提供总结性启示、牌面依据和继续联想问题。
- **自我解读**：单牌解释强调“这张牌在这个问题和牌位里可能提示什么”，保留用户自己的判断空间。
